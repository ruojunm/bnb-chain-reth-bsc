use super::{bsc::bsc_mainnet, bsc_chapel::bsc_testnet, bsc_local::bsc_local, BscChainSpec};
...
impl ChainSpecParser for BscChainSpecParser {
    ...
    const SUPPORTED_CHAINS: &'static [&'static str] = &["bsc", "bsc-testnet", "bsc-local"];
...
pub fn chain_value_parser(s: &str) -> eyre::Result<Arc<BscChainSpec>> {
    match s {
        ...
        "bsc-local" => Ok(Arc::new(BscChainSpec { inner: bsc_local() })),
